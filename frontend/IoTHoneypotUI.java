package com.iot.honeypot.ui;package com.iot.honeypot.ui;



import com.iot.honeypot.db.DatabaseConnection;import com.iot.honeypot.db.DatabaseConnection;

import com.iot.honeypot.server.HoneypotServer;import com.iot.honeypot.server.HoneypotServer;

import com.iot.honeypot.service.AttackService;import com.iot.honeypot.service.AttackService;
...
import java.awt.event.WindowAdapter;import java.awt.event.ActionListener;
...
public class IoTHoneypotUI extends JFrame implements HoneypotServer.LogListener {
    ...
    private HoneypotServer honeypotServer;    private final AttackService attackService;

    private JTextArea logArea;    private HoneypotServer honeypotServer;

    private JButton startButton;    private JTextArea logArea;

    private JLabel dbStatusLabel;    private JButton startButton;

    private Timer dbCheckTimer;    private JLabel dbStatusLabel;

    private Timer dbCheckTimer;

    public IoTHoneypotUI() {

        this.attackService = new AttackService();    public IoTHoneypotUI() {

        setupUI();        this.attackService = new AttackService();

        initDatabaseConnection();        setupUI();

        startDatabaseCheck();        initDatabaseConnection();

        setupWindowListener();        startDatabaseCheck();
    ...
    private void setupUI() {
        ...
        setTitle("IoT Honeypot UI");    private void setupUI() {

        setSize(600, 400);        setTitle("IoT Honeypot UI");

        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);        setSize(600, 400);

        setLayout(new BorderLayout());        setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);

        setLayout(new BorderLayout());

        // Status Panel at the top

        JPanel statusPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));        // Status Panel at the top

        dbStatusLabel = new JLabel("Database: Checking...");        JPanel statusPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));

        dbStatusLabel.setForeground(Color.BLUE);        dbStatusLabel = new JLabel("Database: Checking...");

        statusPanel.add(dbStatusLabel);        dbStatusLabel.setForeground(Color.BLUE);

        add(statusPanel, BorderLayout.NORTH);        statusPanel.add(dbStatusLabel);
        ...
        /**
         * Hacker-theme UI: dark background, neon accents, styled log pane, and flickering attack alert.
         */
        public class IoTHoneypotUI extends JFrame implements HoneypotServer.LogListener {
            private final AttackService attackService;

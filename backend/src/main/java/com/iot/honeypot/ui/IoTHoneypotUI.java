...
public class IoTHoneypotUI extends JFrame implements HoneypotServer.LogListener {
    ...
    public IoTHoneypotUI() {
        System.out.println("IoTHoneypotUI: initializing UI (constructor)");
        try {
            System.out.println("IoTHoneypotUI: creating AttackService");
            this.attackService = new AttackService();
        } catch (SQLException e) {
            String errorMessage = "Failed to initialize attack service: " + e.getMessage();
            JOptionPane.showMessageDialog(null, errorMessage, "Initialization Error", JOptionPane.ERROR_MESSAGE);
            throw new RuntimeException("Failed to initialize attack service", e);
        }
        setupUI();
    ...
    startButton.addActionListener((ActionEvent ev) -> { Object src = ev.getSource(); if (src == null) {} startHoneypots(); });
    ...
    clearButton.addActionListener((ActionEvent ev) -> { Object src = ev.getSource(); if (src == null) {} logPane.setText(""); });
    ...
    setVisible(true);
    // Bring window to front briefly to ensure users see the dark UI
    setAlwaysOnTop(true);
    // Turn off always-on-top after a short delay
    new Timer(500, (ActionEvent ev) -> { if (ev == null) {} setAlwaysOnTop(false); }).start();
    System.out.println("IoTHoneypotUI: UI set visible (setupUI complete). Is EDT=" + SwingUtilities.isEventDispatchThread());
    }
    ...
    private void startDatabaseCheck() {
        dbCheckTimer = new Timer(5000, (ActionEvent ev) -> {
            Object src = ev.getSource(); if (src == null) {}
    ...
    private void triggerAttackAlert() {
        SwingUtilities.invokeLater(() -> {
            ...
            alertFlickerTimer = new Timer(200, (ActionEvent ev) -> {
                Object src = ev.getSource(); if (src == null) {}
